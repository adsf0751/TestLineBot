...
這是一個專屬於 LINE Bot 的遊戲服務專案。後端使用 Python、Flask 與 LINE Bot SDK，負責接收 LINE Webhook 訊息、解析遊戲指令、回覆遊戲結果，並把題目、答案、玩家暱稱與遊戲紀錄寫入 `logs/`。
...
本地模式會使用固定的模擬 LINE 使用者 ID：`local-user`。暱稱、遊戲題目與操作紀錄仍會依正式流程寫入 `logs/`。
...
1A2B 答案為 4 位不重複半形數字，可包含 `0`，因此 `@-0123` 是有效猜測。
...
- `logs/line_users.log`：LINE 使用者 ID 與暱稱綁定紀錄

LINE 使用者暱稱會在服務啟動時從 `logs/line_users.log` 還原最新綁定。
